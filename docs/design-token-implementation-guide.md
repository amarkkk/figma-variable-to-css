...
**Export plugin:** [Variable to CSS](https://github.com/amarkkk/figma-variable-to-css/tree/experimental)
...
| **Grid proportions** | Pre-calculated fractional widths (whole, three-quarters, two-thirds, half, third, quarter) derived from column count on a 12-column grid. In the CSS export, these translate to column span numbers (12, 9, 8, 6, 4, 3), not pixel values. |
...
4. **Value-based responsivity is handled automatically.** You do not need to add media queries for value scaling (font sizes, spacing, padding, dimensions). The tokens.css file already contains breakpoint-specific overrides for values that change across viewports. Values update automatically.
...
```
  ...
  |
  v
  Variable to CSS Plugin  (experimental branch)
...
The CSS export uses the [Variable to CSS plugin](https://github.com/amarkkk/figma-variable-to-css/tree/experimental) (experimental branch). It reads all Figma Variable collections and generates a single CSS file.
...
```css
/* Foundation values - Desktop (default) */
:root {
...
> **Note on breakpoints and media query direction:** The current plugin exports use `max-width` media queries with Desktop as the `:root` default (desktop-first). The breakpoint values themselves are currently hardcoded in the plugin. A planned plugin iteration will make breakpoints configurable (pulled from Figma's `Dimension - 1. Foundations / grid/proportions/viewport` values) and allow choosing between `max-width` (desktop-first) and `min-width` (mobile-first) output. See Section 9 for plugin iteration notes.
...
**Composite tokens / Text Styles:** In Figma, Text Styles combine family + weight + size + line-height into named styles. These serve as the Mapping layer for typography - there is no separate Mapping collection. The plugin can export Text Styles as CSS classes. Whether the frontend team creates SCSS mixins from these is a developer-side decision.
...
In the CSS export, these proportions translate to **column span numbers** and **fr units**, not pixel values:

```css
/* From the fluid export - grid proportions as layout intent */
...
**Aliases Extended** encodes layout intent that changes proportionally across breakpoints:

```css
/* Desktop: CTA content takes half the page container */
:root {
  --dimension-cta-content-horizontal-width:
    var(--dimension-grid-proportions-third--min);
}

/* Tablet: CTA content expands to two-thirds */
@media (max-width: 1365px) {
  :root {
    --dimension-cta-content-horizontal-width:
...
/* Mobile: CTA content expands to three-quarters */
@media (max-width: 839px) {
  :root {
    --dimension-cta-content-horizontal-width:
      var(--dimension-grid-proportions-three-quarters--min);
  }
}
```
...
The current export is fixed mode - discrete values at four breakpoints:

```css
:root { --token: 36px; }                                  /* Desktop (default) */
@media (max-width: 1679px) { :root { --token: 36px; } }   /* Laptop */
@media (max-width: 1365px) { :root { --token: 32px; } }   /* Tablet */
@media (max-width: 839px)  { :root { --token: 32px; } }   /* Mobile */
```

> **Note:** The breakpoint values and media query direction (max-width vs min-width) are pending a plugin iteration. The current values are hardcoded in the plugin. See Section 9 for planned improvements.

### Grid proportions - layout intent, not pixel values

Grid proportion tokens express layout intent using the 12-column grid. In the CSS export, they output as **column span numbers** and **fr units**:
...
- Exports grid proportions as column span numbers and `fr` units (in fluid mode)
- Exports Text Styles as CSS classes (optional)
- Handles unitless values (column count, font weights)

**Planned plugin iteration:**
- Configurable breakpoints pulled from Figma Variables (`Dimension - 1. Foundations / grid/proportions/viewport`) instead of hardcoded values
- Selectable media query direction: `max-width` (desktop-first) vs. `min-width` (mobile-first)
- More granular export controls to select which output modes and features are included
- Resolution of current inconsistency where some fluid exports include piecewise clamp for tokens that should only use grid proportions
...
*This document is a living reference. It will be updated as the token system matures - particularly when the Color Alias layer is rebuilt, the export plugin is iterated, and breakpoint values are finalized.*
