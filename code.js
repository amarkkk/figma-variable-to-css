...
var windowSizePersistTimer = null;
function clampWindowSize(size) {
    var minWidth = 900;
    var minHeight = 600;
    var width = typeof (size === null || size === void 0 ? void 0 : size.w) === 'number' ? size.w : currentWindowSize.w;
    var height = typeof (size === null || size === void 0 ? void 0 : size.h) === 'number' ? size.h : currentWindowSize.h;
    return {
...
// Restore theme
figma.clientStorage.getAsync('theme').then(function (theme) {
    if (theme)
        figma.ui.postMessage({ type: 'theme-loaded', theme: theme });
}).catch(function () { });
// ============================================
// MESSAGE HANDLERS
// ============================================
...
function matchesAsSegment(text, keyword) {
    var pos = 0;
...
function generatePaintStyleName(name) {
    return name
        .replace(/\//g, '-')
        .replace(/\s+/g, '-')
        .replace(/[^a-zA-Z0-9-]/g, '')
...
async function generatePaintStyleCSS(options) {
    var lines = [];
    var paintStyles = await figma.getLocalPaintStylesAsync();
...
function generateEffectStyleName(name) {
    return name
        .replace(/\//g, '-')
        .replace(/\s+/g, '-')
        .replace(/[^a-zA-Z0-9-]/g, '')
...
async function generateEffectStyleCSS(options) {
    var lines = [];
    var effectStyles = await figma.getLocalEffectStylesAsync();
